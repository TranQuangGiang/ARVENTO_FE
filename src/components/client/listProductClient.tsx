...
const ListProductClient = () => {
  ...
  const [allProductSlice, setAllProductSlice] = useState<any[]>([]);
  const [allProductSliceCategory, setAllProductSliceCategory] = useState<any[]>([]);

  const { data: allProductData, isFetching: isFetchingAll } = useListClient({
    resource: `/products?page=${page}`,
  });
  useEffect(() => {
    // Nếu có dữ liệu trả về và không đang trong quá trình fetch
    if (allProductData && !isFetchingAll) {
      const fetchedDocs = allProductData.data.docs || [];
      const hasNextPage = allProductData.data.hasNextPage;
      
      const existingIds = new Set(allProducts.map(p => p._id));
      // Filter out any products that already exist in our state
      const uniqueNewProducts = fetchedDocs.filter(
        (product: any) => !existingIds.has(product._id)
      );

      setAllProducts((prevProducts) => [...prevProducts, ...uniqueNewProducts]);
      if (hasNextPage) {
        setPage((prevPage) => prevPage + 1);
      }
    }
  }, [allProductData, isFetchingAll]);
  ...
  useEffect(() => {
    ...
    const sortedAll = [...products]
      .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
      .slice(0, 12);
    setAllProductSlice(sortedAll);
  }, [allProducts])
  
  // sản phẩm danh mục
  const { data: categoryProductData, isFetching: isFetchingCategory } = useListClient({
    resource: `/products?category_id=6843e798c4fb85b25844b4a2&page=${categoryPage}`,
  });
  useEffect(() => {
    if (categoryProductData && !isFetchingCategory) {
      const fetchedProductDocs = categoryProductData.data.docs || [];
      const hasNextPageProduct = categoryProductData.data.hasNextPage;

      const existingIds = new Set(allProductsCategory.map(p => p._id));

      const uniqueNewProducts = fetchedProductDocs.filter(
        (product: any) => !existingIds.has(product._id)
      );
      setAllProductsCategory((prev) => [...prev, ...uniqueNewProducts])

      if (hasNextPageProduct) {
        setCategoryPage((prevPages) => prevPages + 1);
      }
    }
  }, [categoryProductData, isFetchingCategory]);
  useEffect(() => {
    const productCategory = allProductsCategory.filter((p:any) => p.isActive);

    const sortedProductCategory = [...productCategory]
    .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
    .slice(0, 4);
    setAllProductSliceCategory(sortedProductCategory);
  }, [allProductsCategory]);
